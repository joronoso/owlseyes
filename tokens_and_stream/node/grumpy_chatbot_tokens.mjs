...
// Array to store number of tokens for each message.
...
while (text_user = (await rl.question('What do you want to say to the grumpy chatbot?\n> ')).trim()) {
    ...
    // Uncomment to display the size per message, and comparison with that returned by "usage".
    /*
    console.log(`--- ${tokens_message.reduce(((a, b) => a + b))} tokens of ${window}. Max response is ${max_response} ---`)
    for (let i = 0; i < messages.length; i++) {
        console.log(`${messages[i].role} - ${messages[i].content.substring(0, 10)} - ${tokens_message[i]}`);
    }
    console.log(`Prompt usage: ${response.usage.prompt_tokens} - Completion usage: ${response.usage.completion_tokens}`)
    */
}
