...
while True:
    ...
    # print('--- '+str(sum(tokens_message))+' tokens of '+str(ventana)+'. Max response is '+str(max_response)+' ---')
    # for i in range(len(messages)):
    #     print(messages[i]['role']+' - '+messages[i]['content'][:10] + ' - ' + str(tokens_message[i]))
    # print('Prompt usage: '+str(respuesta.usage.prompt_tokens)+' - Completion usage: '+str(respuesta.usage.completion_tokens))